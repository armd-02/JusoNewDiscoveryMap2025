...
class Maplibre {
    ...
    addPolygon(data, target, titleTag) {
        ...
        if (source !== undefined) {
            ...
            if (Conf.polygon.textSize > 0) {        // fontsizeが0より上の場合
                mapLibre.map.addLayer({
                    id: target + "-text", type: 'symbol', source: target,
                    layout: {
                        "text-field": titleTag,             // 指定されたルールに沿う
                        "text-font": Conf.polygon.textFont,     // 使用可能なフォント（spriteに依存）
                        "text-size": Conf.polygon.textSize,
                        "text-anchor": Conf.polygon.textAnchor,            // テキストの位置（上、中央、下など）
...
class GeoCont {
    ...
    writePolygon(geojson) {
        ...
        const params = { id: layerId, type: "fill", source: sourceId, paint: Conf.polygon.fill }
